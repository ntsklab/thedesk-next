...
const generateNotification = (notification: Entity.Notification, formatMessage: (descriptor: MessageDescriptor, values?: any, opts?: any) => string): [string, string] => {
	switch (notification.type) {
		case 'follow':
			return [formatMessage({ id: 'timeline.notification.follow.title' }), formatMessage({ id: 'timeline.notification.follow.body' }, { user: notification.account.acct })]
		case 'move':
		...
		case 'status':
			return [formatMessage({ id: 'timeline.notification.status.title' }), formatMessage({ id: 'timeline.notification.status.body' }, { user: notification.account.acct })]
		case 'update':
