...
const AutoCompleteTextarea: React.ForwardRefRenderFunction<HTMLTextAreaElement, ArgProps> = (props, ref) => {
	...
	const onKeyDown: KeyboardEventHandler<HTMLInputElement> = (ea) => {
		const e: any = ea
		if (opened) {
			if (e.code === 'ArrowDown') {
		...
		<>
			...
			<Whisper
				...
				speaker={({ className, left, top }, ref) => (
					<AutoCompleteList className={className} left={left} top={top} data={suggestList} onSelect={insertItem} highlight={highlight} setHighlight={setHighlight} ref={ref} />
				)}
				ref={triggerRef}
				trigger="click"
